...
#Coloca el número de cada línea
for file in *.1.csv; do awk '{print NR "," $s}' $file > "$(basename "$file" .csv).2.csv"; done       
#Coloca el nombre del archivo en cada fila
...
#Agregar coma después de la mayúscula
...
#Para reemplazar los .1.2 por nada
sed -e 's/.1.2//g' data_final.csv > Resultado.1    
#Elimina los archivos sobrantes
